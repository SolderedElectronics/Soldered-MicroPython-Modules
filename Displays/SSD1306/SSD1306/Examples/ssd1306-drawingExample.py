...
def test_draw_line():
    ...
    for i in range(0, display.width, 4):
        display.line(
            0, 0, i, display.height - 1, 1
        )  # Draw line (x0,y0 to x1,y1, color)
...
def test_fill_rect():
    ...
    for i in range(0, display.height // 2, 3):
        ...
        time.sleep_ms(1)
        display.fill_rect(
            i, i, display.width - 2 * i, display.height - 2 * i, 0
        )  # Inverse
        display.show()
        time.sleep_ms(1)
...
def test_draw_bitmap():
    ...
    display.blit(
        buf, (display.width - LOGO_WIDTH) // 2, (display.height - LOGO_HEIGHT) // 2
    )
    display.show()
    time.sleep(1)
