...
print(
    "Sats HDOP  Latitude    Longitude    Age  Date       Time     Alt     Course Speed Card Dist(km) Course Card  Chars Sentences Checksum"
)

while True:
    print(
        "{:>4} ".format(gps.satellites.value() if gps.satellites.isValid() else "****"),
        end="",
    )
    print(
        "{:>5.1f} ".format(gps.hdop.hdop()) if gps.hdop.isValid() else "***** ", end=""
    )

    if gps.location.isValid():
        print(
            "{:>10.6f}  {:>10.6f} ".format(gps.location.lat(), gps.location.lng()),
            end="",
        )
    ...
    print(
        "{:>7.2f} ".format(gps.altitude.meters())
        if gps.altitude.isValid()
        else "******* ",
        end="",
    )
    print(
        "{:>6.2f} ".format(gps.course.deg()) if gps.course.isValid() else "****** ",
        end="",
    )
    print(
        "{:>5.2f} ".format(gps.speed.kmph()) if gps.speed.isValid() else "***** ",
        end="",
    )
    print(
        "{:<4} ".format(GNSS.cardinal(gps.course.deg()))
    ...
    if gps.location.isValid():
        distance_km = (
            GNSS.distanceBetween(
                gps.location.lat(), gps.location.lng(), CITY_LAT, CITY_LON
            )
            / 1000
        )
        course_to_city = GNSS.courseTo(
            gps.location.lat(), gps.location.lng(), CITY_LAT, CITY_LON
        )
        print(
            "{:>8.0f} {:>6.2f} {:<4} ".format(
                distance_km, course_to_city, GNSS.cardinal(course_to_city)
            ),
            end="",
        )
    else:
