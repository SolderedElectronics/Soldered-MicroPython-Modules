...
class BME688:
    ...
    def begin(self, mode=None, oss=None):
        ...
        if mode is None and oss is None:
            ...
            self.i2c_execute(BME_688_CTRL_MEAS_REG, self.temp_oss << 5 | self.press_oss << 2 | BME_688_FORCED_MODE)
            ...
            if mode <= BME_688_PARALLEL_MODE:
                ...
                self.i2c_execute(BME_688_CTRL_MEAS_REG, BME_688_OSS_1 << 5 | BME_688_OSS_1 << 2 | mode)
                self.i2c_execute(BME_688_IIR_FILTER_REG, BME_688_IIR_FILTER_C15)
                self.readCalibParams()
            else:
    ...
    def setHeatProfiles(self):
        ...
        for i in range(9):
            ...
            self.i2c_execute(BME_688_GAS_WAIT_PROFILE_REG + i, 
    ...
    def isConnected(self):
        ...
        try:
            ...
            self.printLog("I2C device not found at address 0x{:02X}".format(self._address))
            return False
        ...
        if isinstance(chip_id, bytes):
            if chip_id == expected_id:
                return True
            self.printLog("Unexpected chip ID: received {} expected {}".format(chip_id, expected_id))
    ...
    def readUCTemp(self, adc_T):
        ...
        var2 = ((adc_T / 131072.0 - self.par_t16[0] / 8192.0) ** 2) * (self.par_t3 * 16.0)
    ...
    def readUCPres(self, adc_P):
        ...
        var1 = (((self.par_p8[0] * var1 * var1) / 16384.0) + (self.par_p16[1] * var1)) / 524288.0  # p3 and p2
        ...
        var3 = (press_comp / 256.0) * (press_comp / 256.0) * (press_comp / 256.0) * (self.par_p8[3] / 131072.0)  # p10
        
        self.p_fine = press_comp + (var1 + var2 + var3 + (self.par_p8[2] * 128.0)) / 16.0  # p7
    ...
    def readUCHum(self, adc_H):
        ...
        var2 = var1 * ((self.par_h16[1] / 262144.0) * 
                      (1.0 + (self.par_h8[1] / 16384.0) * temp + 
    ...
    def readUCGas(self, target_temp):
        ...
        var5 = var4 + (var3 * temp)
        self.g_fine = int(3.4 * ((var5 * (4.0 / (4.0 + ((self.res_heat_range & BME_688_HEAT_RANGE_MASK) >> 4)) *
    ...
    def startGasMeasurement(self, profile, waitTime):
        ...
        # Trigger measurement
        if not self.i2c_execute(BME_688_CTRL_MEAS_REG, 
    ...
    def readGas(self, profile):
        ...
        """
        if 0 <= profile < 10:
            return self.startGasMeasurement(profile, 
