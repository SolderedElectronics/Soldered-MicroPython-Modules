# FILE: APDS9960.py
# AUTHOR: Thomas Liske (https://github.com/liske)
# MODIFIED BY: Josip Šimun Kuči @ Soldered
# BRIEF: A MicroPython module for the APDS9960 gesture, proximity and light sensor
...
class APDS9960:
    ...
    def __init__(self, i2c=None, address=APDS9960_I2C_ADDR, valid_id=APDS9960_DEV_ID):
        # I2C stuff
            ...
            if fifo_level > 0:
                ...
                if len(fifo_data) >= 4:
                    for i in range(0, len(fifo_data), 4):
                        self.gesture_data_.u_data[self.gesture_data_.index] = fifo_data[
                            i + 0
                        ]
                        self.gesture_data_.d_data[self.gesture_data_.index] = fifo_data[
                            i + 1
                        ]
                        self.gesture_data_.l_data[self.gesture_data_.index] = fifo_data[
                            i + 2
                        ]
                        self.gesture_data_.r_data[self.gesture_data_.index] = fifo_data[
                            i + 3
                        ]
    ...
    # reads the red light level and returns its RGB value
    def readRedLight(self):
        self.setMode(APDS9960_MODE_AMBIENT_LIGHT)
        ...
        return min(int(float(r / 10000) * 255), 255)

    # reads the green light level and returns its RGB value
    def readGreenLight(self):
        ...
        return min(int(float(g / 10000) * 255), 255)

    # reads the blue light level and returns its RGB value
    def readBlueLight(self):
        ...
        return min(int(float(b / 10000) * 255), 255)
    ...
    def processGestureData(self):
        ...
        if (
            self.gesture_data_.total_gestures <= 32
            and self.gesture_data_.total_gestures > 0
        ):
