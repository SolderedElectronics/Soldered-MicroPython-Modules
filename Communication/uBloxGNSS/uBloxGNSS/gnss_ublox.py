...
class SolderedGNSS:
    ...
    def __init__(self):
        ...
        self._pvt = {
            ...
            'valid': 0, 'flags': 0, 'flags2': 0,
            'fixType': 0, 'numSV': 0,
            'lon': 0, 'lat': 0, 'height': 0, 'hMSL': 0,
            'hAcc': 0, 'vAcc': 0,
            'gSpeed': 0, 'headMot': 0, 'sAcc': 0, 'headAcc': 0,
            'pDOP': 0,
        }

        # Cached NAV-TIMELS fields
        self._timels = {
            'srcOfCurrLs': 0, 'currLs': 0,
            'lsChange': 0, 'timeToLsEvent': 0,
            'valid': 0,
        }
    ...
    def begin(self, i2c, addr=0x42, timeout_ms=1100):
        ...
        return self._pollAndWait(_CLS_NAV, _ID_NAV_PVT, timeout_ms=timeout_ms) is not None
    ...
    def getProtocolVersion(self, timeout_ms=1100):
        ...
        for i in range(ext_count):
            start = 40 + i * 30
            ext = payload[start:start + 30].split(b'\x00')[0].decode('ascii', 'ignore')
    ...
    def getModuleInfo(self, timeout_ms=1100):
        ...
        if payload is None or len(payload) < 40:
            return False
        self.minfo['swVersion'] = payload[0:30].split(b'\x00')[0].decode('ascii', 'ignore')
        self.minfo['hwVersion'] = payload[30:40].split(b'\x00')[0].decode('ascii', 'ignore')
        ext_count = (len(payload) - 40) // 30
        exts = []
        for i in range(ext_count):
            start = 40 + i * 30
    ...
    def _getCfgPrt(self, timeout_ms=1100):
        ...
        return self._pollAndWait(_CLS_CFG, _ID_CFG_PRT,
    ...
    def powerOff(self, duration_ms):
        ...
        self._sendUBX(_CLS_RXM, _ID_RXM_PMREQ,
                      struct.pack('<II', duration_ms, 0x02))

    def powerOffWithInterrupt(self, duration_ms,
