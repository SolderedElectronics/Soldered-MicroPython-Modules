...
def dlc2len(dlc):
    ...
    if dlc <= CAN_DLC_8:
        return dlc
    return {CAN_DLC_12: 12, CAN_DLC_16: 16, CAN_DLC_20: 20, CAN_DLC_24: 24,
            CAN_DLC_32: 32, CAN_DLC_48: 48, CAN_DLC_64: 64}.get(dlc, 0)

def len2dlc(length):
    """Convert byte count to DLC code."""
    if length <= 8:  return length
    if length <= 12: return CAN_DLC_12
    if length <= 16: return CAN_DLC_16
    if length <= 20: return CAN_DLC_20
    if length <= 24: return CAN_DLC_24
    if length <= 32: return CAN_DLC_32
    if length <= 48: return CAN_DLC_48
    return CAN_DLC_64
...
class MCP2518FD:
    ...
    def _read_byte(self, addr):
        rx = self._transfer(bytes([(_INSTR_READ << 4) | ((addr >> 8) & 0xF),
                                    addr & 0xFF, 0x00]))
        return rx[2]

    def _write_byte(self, addr, val):
        self._transfer(bytes([(_INSTR_WRITE << 4) | ((addr >> 8) & 0xF),
                               addr & 0xFF, val & 0xFF]))
    ...
    def _read_word_array(self, addr, n_words):
        ...
        for i in range(n_words):
            o = i * 4
            words.append(raw[o] | (raw[o+1] << 8) | (raw[o+2] << 16) | (raw[o+3] << 24))
        return words
    ...
    def _tx_fifo_configure(self, channel, fifo_size=7, payload_size=CAN_PLSIZE_64,
                            priority=1, rtr_enable=0, attempts=0):
    ...
    def _rx_fifo_configure(self, channel, fifo_size=15, payload_size=CAN_PLSIZE_64, timestamp=0):
        word = _canFifoResetValues[0]
        word &= ~(1 << 7)
    ...
    def _filter_to_fifo_link(self, filt, channel, enable=True):
        self._write_byte(_REG_CiFLTCON + filt,
                         (channel & 0x1F) | (0x80 if enable else 0x00))
        ...
        if self._data_factor <= 1:
            ...
            while tq <= max_tq + 1 and brp > 0:
                if 4 <= tq <= max_tq:
                    ...
                    if err <= smallest_err:
                        smallest_err = err; best_brp = brp; best_tq = tq
                if 3 <= tq < max_tq:
                    err = arb_bitrate * (tq + 1) * brp - self._sys_clock
                    if err <= smallest_err:
                        smallest_err = err; best_brp = brp; best_tq = tq + 1
                brp -= 1
                tq = (self._sys_clock // arb_bitrate // brp) if brp else max_tq + 1

            ps2 = best_tq // 5
            if ps2 == 0: ps2 = 1
            elif ps2 > MAX_ARB_PS2: ps2 = MAX_ARB_PS2
            ps1 = best_tq - ps2 - 1
            if ps1 > MAX_ARB_PS1:
                ps2 += ps1 - MAX_ARB_PS1
                ps1 = MAX_ARB_PS1
    ...
    def _tx_channel_load(self, channel, id_word, ctrl_word, data, flush):
        ...
        buf[0] = id_word & 0xFF;          buf[1] = (id_word >> 8) & 0xFF
        buf[2] = (id_word >> 16) & 0xFF;  buf[3] = (id_word >> 24) & 0xFF
        buf[4] = ctrl_word & 0xFF;        buf[5] = (ctrl_word >> 8) & 0xFF
        buf[6] = (ctrl_word >> 16) & 0xFF; buf[7] = (ctrl_word >> 24) & 0xFF
        buf[8:] = data
    ...
    def _tx_message_queue(self, id_word, ctrl_word, data):
        for _ in range(_MAX_TXQUEUE_ATTEMPTS):
            if self._tx_channel_event_get(_APP_TX_FIFO) & CAN_TX_FIFO_NOT_FULL_EVENT:
                return self._tx_channel_load(_APP_TX_FIFO, id_word, ctrl_word, data, True)
        return -2
    ...
    def _init(self, speedset, clock):
        ...
        self._tx_fifo_configure(_APP_TX_FIFO, fifo_size=7,  payload_size=CAN_PLSIZE_64, priority=1)
    ...
    def _send_msg(self, data, can_id, ext, rtr):
        ...
        ctrl_word  = dlc & 0xF
        if ext: ctrl_word |= (1 << 4)
        if rtr: ctrl_word |= (1 << 5)
        ctrl_word |= (1 << 6)            # BRS
        if n > 8: ctrl_word |= (1 << 7)  # FDF

        err = self._tx_message_queue(id_word, ctrl_word, bytes(data[:n]))
        if err < 0:
            if err == -2: return CAN_SENDMSGTIMEOUT
            if err == -3: return CAN_FAILTX
            return CAN_FAILINIT
        return CAN_OK
    ...
    def readMsgBuf(self, buf=None):
        ...
        """
        self.can_id, self.ext_flg, self.rtr, dlc, data = \
            self._rx_message_get(_APP_RX_FIFO, 64)
