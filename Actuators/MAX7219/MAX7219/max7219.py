...
class MAX7219:
    ...
    def _set_module_parameters(self, mod):
        ...
        if mod in (DR0CR0RR0_HW,):
            self._hw_dig_rows = False
            self._hw_rev_cols = False
            self._hw_rev_rows = False
        elif mod in (DR0CR0RR1_HW,):
            self._hw_dig_rows = False
            self._hw_rev_cols = False
            self._hw_rev_rows = True
        elif mod in (DR0CR1RR0_HW, GENERIC_HW):
            self._hw_dig_rows = False
            self._hw_rev_cols = True
            self._hw_rev_rows = False
    ...
    def getPoint(self, r, c):
        ...
        if self._hw_dig_rows:
            return bool(
                self._matrix[buf]["dig"][self._hw_row(r)] & (1 << self._hw_col(c_loc))
            )
        else:
            return bool(
                self._matrix[buf]["dig"][self._hw_row(c_loc)] & (1 << self._hw_col(r))
            )
    ...
    def transform(self, ttype, start_dev=None, end_dev=None):
        ...
        if ttype == TSL:
            ...
            for buf in range(end_dev, start_dev - 1, -1):
                ...
                next_col = (
                    self.getColumn(buf - 1, COL_SIZE - 1)
                    if buf > start_dev
                    else col_data
                )
